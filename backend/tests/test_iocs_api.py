...
def test_get_ioc_enrichment():
    ...
    response = client.get(f"/iocs/{ioc_id}/enrichment")
    assert response.status_code == 200
    assert response.json()["risk_score"] == 92


def test_get_ioc_enrichment_not_found():
    response = client.get("/iocs/00000000-0000-0000-0000-000000000000/enrichment")
    assert response.status_code == 404


def test_search_iocs_by_value():
    response = client.get("/iocs/search?value=185.220")
    assert response.status_code == 200
    assert isinstance(response.json(), list)


def test_search_iocs_by_min_risk():
    response = client.get("/iocs/search?min_risk=90")
    assert response.status_code == 200